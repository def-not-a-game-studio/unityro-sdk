...
namespace UnityRO.Core.Camera {
    ...
    public class CharacterCamera : MonoBehaviour {
        ...
        [Header(":: Refs")] public UnityCamera GameCamera;

        [Header(":: User Parameters")] public Vector2 MouseSensitivity = Vector2.one;
        public float ScrollPitchSensitivity = 1f;
        public float ScrollZoomSensitivity = 1f;

        [Header(":: Settings")] public CameraControlProfile YawControl;
        ...
        private void RecomputeHorizontalDirection() {
            HorizontalDirection = new Vector3(Mathf.Cos(m_Yaw) * m_SphereSliceRadius, 0f,
                                              Mathf.Sin(m_Yaw) * m_SphereSliceRadius);
        }
        ...
        public void OnRotatePressed(Vector2 mouseDelta, float mouseScroll, bool isShiftModified) {
            if (isShiftModified) {
                var vScroll = mouseDelta.y * MouseSensitivity.y + mouseScroll * ScrollPitchSensitivity;
                if (vScroll == 0f) return;

                Pitch -= vScroll * Time.deltaTime;
                RecomputeCameraAngle();
            } else {
                YawControl.SetInertia(mouseDelta.x * MouseSensitivity.x);
            }
            
            var dt = Time.deltaTime;
            if (!YawControl.Update(dt)) return;
        ...
        public void OnRotateReleased(bool isShiftModified) {
            if (!isShiftModified) {
                YawControl.Release();
            }
        }

        public void OnZoom(float mouseScroll, bool isShiftModified) {
            var dt = Time.deltaTime;
            if (!isShiftModified || mouseScroll != 0f) {
